...
"""Turn v86's raw encoding corpus into the decoded instruction database.
    ...
    x86/<target>/instructions/decoded_database/<target>.corpus.jsonl   (in)
        one line per encoding shape, up to two full 16-byte samples each
    x86/<target>/instructions/decoded_database/<target>.decoded.jsonl  (out)
    x86/<target>/instructions/decoded_database/<target>.decoded.tsv    (out)
    x86/<target>/instructions/decoded_database/README.md               (out)
...
Usage:  .venv/bin/python tools/decode_db.py --target winnt|win98
"""
...
def build(corpus_paths, out_dir, target, max_variants=2):
    ...
    with open(os.path.join(out_dir, "%s.decoded.jsonl" % target), "w") as f:
        for r in recs:
            f.write(json.dumps(r, separators=(",", ":")) + "\n")

    write_tsv(os.path.join(out_dir, "%s.decoded.tsv" % target), recs)
...
GUEST = {"winnt": "Windows NT 4.0", "win98": "Windows 98",
         "winnt35": "Windows NT 3.5", "winnt31": "Windows NT 3.1"}


def write_readme(path, recs, stats, problems, target):
    guest = GUEST.get(target, target)
    ...
    with open(path, "w") as f:
        w = f.write
        w("# Decoded instruction database — %s boot\n\n" % guest)
        ...
        w("Each row is a **distinct instruction byte string** that %s\n"
        ...
        w("## Files\n\n")
        w("| file | content |\n|---|---|\n")
        w("| `%s.corpus.jsonl` | raw corpus from the emulator: encoding shape -> up to 2 sample byte strings + execution count |\n" % target)
        w("| `%s.decoded.jsonl` | one JSON record per distinct byte string, all fields |\n" % target)
        w("| `%s.decoded.tsv` | the same, flat; the `fields` column is `name@bitoffset:width=value` |\n" % target)
        w("| `%s.l8_expect.csv` | the l8 test vector: bytes in -> expected `decode(d)` arm, bound arguments and `end` (produced by `x86/tools/l8_expect.py`) |\n" % target)
        w("| `%s.l8_gaps.md` | which encodings the l8 arm list does not yet cover, and the `end` verdict |\n\n" % target)
        ...
        w("The corpus is collected on v86's **interpreter** path. v86 only\n"
          ...
          "frequencies use `%s.hist.tsv`, which also instruments the JIT.\n"
          % target)
...
def main():
    ...
    ap.add_argument("--target", default="winnt",
                    help="guest id as in targets.json (winnt, win98, ...)")
    ap.add_argument("--corpus", nargs="*", default=None,
                    help="corpus JSONL files in phase order; "
                         "default: <out-dir>/<target>.corpus[.phaseN].jsonl")
    ap.add_argument("--out-dir", default=None,
                    help="default: x86/<target>/instructions/decoded_database")
    ...
    out_dir = a.out_dir or os.path.join(here, "..", a.target, "instructions",
                                        "decoded_database")

    paths = a.corpus
    if not paths:
        pat = os.path.join(out_dir, "%s.corpus.phase*.jsonl" % a.target)
        ph = sorted(glob.glob(pat),
                    key=lambda p: int(p.rsplit("phase", 1)[1].split(".")[0]))
        final = os.path.join(out_dir, "%s.corpus.jsonl" % a.target)
        paths = ph + ([final] if os.path.exists(final) else [])
        if not paths:
            sys.exit("no corpus found in %s" % out_dir)
    print("corpus: " + ", ".join(os.path.basename(p) for p in paths))

    recs, stats, problems = build(paths, out_dir, a.target, a.max_variants)
