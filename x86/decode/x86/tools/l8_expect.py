...
Reads the decoded database (`<target>.decoded.jsonl`, itself cross-checked against
iced-x86) and, for every instruction the guest executed, states what that l8
file *should* produce:
...
executes that this deliberately-partial decoder does not yet cover.
    ...
    ../.venv/bin/python l8_expect.py --target winnt|win98
"""
...
def main():
    ...
    dbdir = os.path.join(here, "..", a.target, "instructions",
                         "decoded_database")
    db = a.db or os.path.join(dbdir, "%s.decoded.jsonl" % a.target)
    out = a.out or os.path.join(dbdir, "%s.l8_expect.csv" % a.target)
    ...
    rep = os.path.join(dbdir, "%s.l8_gaps.md" % a.target)
    write_gaps(rep, n, covered, end_bad, bad_examples, uncovered,
               dyn_cov, dyn_unc, a.target)
    print("\nwrote " + out)
    print("wrote " + rep)
...
def write_gaps(path, n, covered, end_bad, bad, uncovered, dyn_cov, dyn_unc,
    ...
    with open(path, "w") as f:
        w = f.write
        w("# `decode_x86_withdecode_extended_operands.l8` vs. a %s boot\n\n"
          % guest)
        ...
        w("These are not subset decisions. The variant HAS arms in \u00a73, but \u00a73\n"
        ...
        # A variant is "partially covered" when some of its encodings reached an
        # arm and others did not: that distinguishes a missing *arm* from a
        # deliberately omitted *variant*, which is the difference worth naming.
        armed = {r[0] for r in ARMS}
        part = {}
        for (v, k), c in uncovered.items():
            if v in armed:
                part.setdefault(v, []).append((k, c))
        if part:
            w("| variant | arms in \u00a73 | missing modrm kinds |\n|---|---|---|\n")
            for v in sorted(part, key=lambda v: -sum(c for _, c in part[v])):
                have = [g or "(none)" for n, g, _, _, _ in ARMS if n == v]
                miss = ", ".join("`%s` (%d)" % (k, c)
                                 for k, c in sorted(part[v], key=lambda x: -x[1]))
                w("| `%s` | %s | %s |\n" % (v, ", ".join(have), miss))
        else:
            w("None: every variant with an arm has an arm for every addressing\n"
              "mode the guest used.\n")
