...
* 7.22 ew          	9/3/09      - Fix: Corrected tm_rb_n_r/f times to use internal busy instead of global p_rb signal
                                    ...
                                    - Fix: Fixed OTP functionality via ONFI Set Features for parts with ECC timing
...
reg [ROW_BITS -1 : 0]           row_addr_bk [0:NUM_PLANES -1]; // for sequential cache read cross block boundary
reg                             cache_seqtl_cross_blk;
...
reg queued_plane_copy [0: NUM_PLANES-1];
...
initial begin
    ...
    saw_cmnd_85h        =   1'b0; //flag for 85h cmd 
`ifdef JEDEC_ARRAY_DEFINED
`else
    ...
    task memory_write;
        ...
        begin
            ...
            case (memory_select)
                0:  begin
`ifdef MODEL_SV
                    mem_array[page_addr]       = (mem_array[page_addr] & ~ld_mask) | (data<<(col  * BPC_MAX* DQ_BITS));
`else
                `ifdef FullMem
                    mem_array[page_addr]       = (mem_array[page_addr] & ~ld_mask) | (data<<(col  * BPC_MAX* DQ_BITS));
                `else
...
//        #tWB_delay
    ...
    if(lastCmd === 8'h35 && FEATURE_SET2[CMD_ECC] == 1) //Internal ECC timing when Internal Data Move operation
        delay = tR_max_ecc + temp_delay;
    else    
...
//    #tWB_delay; // Delay For RB# (tWB)
    ...
    for (eb_thisPlane=0; eb_thisPlane < NUM_PLANES; eb_thisPlane=eb_thisPlane+1) begin
      if(queued_plane[eb_thisPlane]) begin
        eb_lock_addr = row_addr[eb_thisPlane];        
        if (LockInvert) begin
            eb_unlocked_erase = (~ALLOWLOCKCOMMAND) || (((eb_lock_addr < UnlockAddrLower) || (eb_lock_addr > UnlockAddrUpper)) && eb_unlocked_erase);
        end else begin
            eb_unlocked_erase = (~ALLOWLOCKCOMMAND) || (((eb_lock_addr >= UnlockAddrLower) && (eb_lock_addr <= UnlockAddrUpper)) && eb_unlocked_erase);
        end
      end
...
begin
    ...
    row_valid = 1'b0;
    saw_cmnd_85h <= 1'b0; // flag goes low 
...
begin
    ...
    for (thisPlane=0; thisPlane < NUM_PLANES; thisPlane=thisPlane+1) begin
      if (queued_plane[thisPlane]) begin
        ...
        if (LockInvert) begin
            unlocked_write = (~ALLOWLOCKCOMMAND) || (((lock_addr < UnlockAddrLower) || (lock_addr > UnlockAddrUpper)) && unlocked_write);
        end else begin
            unlocked_write = (~ALLOWLOCKCOMMAND) || (((lock_addr >= UnlockAddrLower) && (lock_addr <= UnlockAddrUpper)) && unlocked_write);
        end
      end
    ...
    // ----------------------------------------------------------------------------------------------------
		...
		// Internal ECC timing for Internal Data Move operation
		else if(saw_cmnd_85h == 1'b1 && FEATURE_SET2[CMD_ECC] == 1'b1) delay = tPROG_typ_ecc;		
		else delay = tPROG_typ;
	...
	// need active_plane ? 1 : 0 format instead of doing row_addr[active_plane] b/c doing the latter gave us plane1 data when active_plane was 0, race condtion?
        ...
        //delay = tLBSY_max;
        //go_busy(delay);
        array_prog_done <= #(tLBSY_max) 1'b1;
        Rb_n_int <= #(tLBSY_max) 1'b1;   // not busy anymore
        status_register [6:5] <= #(tLBSY_max) 2'b11;
        if (LOCK_DEVICE) begin
            status_register [7] <= #(tLBSY_max) 1'b0;
        end else begin
            status_register [7] <= #(tLBSY_max) 1'b1;
        end
...
begin
    ...
    if (lastCmd == 8'h80 && ~cmnd_78h && LUN_pgm_clear && ~cmnd_85h) begin
	if(~multiplane_op_wr) begin
	    clear_queued_planes;
	end
	if (bypass_cache)
	   clear_data_register(new_addr[PAGE_BITS + (NUM_PLANES >> 2) : PAGE_BITS]);
	else
	   clear_cache_register(new_addr[PAGE_BITS + (NUM_PLANES >> 2) : PAGE_BITS]);
    end
...
always @ (posedge row_valid) begin
    if (FEATURE_SET[CMD_2PLANE] | FEATURE_SET[CMD_MP]) begin
        ...
        if (~cmnd_78h & (lastCmd !== 8'hFA)) begin
            if ((lastCmd == 'h80) && cmnd_85h)
               $sformat(msg, "Plane address cant be changed when cmnd 85h is issued");
            else
            ...
            if (lastCmd == 8'h06) begin
                ...
                row_addr[active_plane] = {row_addr[active_plane][ROW_BITS-1:(PAGE_BITS+(NUM_PLANES >> 2)+1)],active_plane[(NUM_PLANES >> 2) : 0],row_addr[active_plane][PAGE_BITS-1:0]};
            end else begin
    ...
    if (lastCmd === 8'h24 & ~LOCKTIGHT) begin
        UnlockAddrUpper = {row_addr[active_plane][ROW_BITS-1:PAGE_BITS],{PAGE_BITS{1'b1}}}; // remove page bits
    ...
    if ((lastCmd === 8'hFA) & (die_select == 1)) begin
        if (Rb_n_int == 1'b0) begin
           $sformat(msg, "RESET WHILE BUSY - ABORT");
           INFO(msg);
	end
        nand_reset (1'b1);
	disable go_busy.delay_loop;
    end	
end
...
*/
        ...
        if ((addr_start >= (COL_BYTES +1)) && (addr_start <= addr_stop) && ~nand_mode[0]) begin : latch_row_addr
			...
			die_is_selected;
                    ...
                    if(die_select & (new_addr[PAGE_BITS-1:0] > NUM_PAGE)) begin
...
always @ (posedge We_n) begin : cLatch
    if (command_enable) begin : Cle_enable
        ...
        `ifdef SO
        if (~InitReset_Complete & FEATURE_SET[CMD_ONFI] & ((Io[7:0] != 8'hFF) & (Io[7:0] != 8'hFC)) ) begin : reset_check
        `else
        if (~InitReset_Complete & FEATURE_SET[CMD_ONFI] & (Io[7:0] != 8'hFF) ) begin : reset_check
        `endif
        ...
        if (Rb_n_int === 1'b1) begin : cLatch_unbusy
            ...
            if (Io [7 : 0] === 8'h00) begin : cmnd_00h
                ...
                saw_cmnd_60h_clear = 1'b0;
                stat_to_rd_mode_c0h  =    1'b0;
                if(~nand_mode[0] & ~(lastCmd === 8'h31 || lastCmd == 8'hE0 || lastCmd === 8'h00 || lastCmd === 8'h15) & die_select & status_register[6] & ~status_register[5]) begin $sformat(msg, "ERROR: Read Operation while array busy"); INFO(msg); end   // ??? verify for all models
...
`endif
		...
		read_out = 1'b0;
		    ...
		    // !cmnd_78h is for case of die1 78h-00h, die0 78h-31h; die 0 should auto-increment row addr
		    	...
		    	for (i=0;i<NUM_PLANES; i=i+1) begin
                            queued_plane_copy[i] = 1'b0;
                        end
			cache_seqtl_cross_blk = 1'b0;
                        ...
                        for (plane_addr = 0; plane_addr < NUM_PLANES; plane_addr = plane_addr +1) begin
                            if (queued_plane[plane_addr]) begin
                                ...
                                if(~(&row_addr[plane_addr][PAGE_BITS-1:0])) begin
                                    ...
                                    row_addr_bk[(plane_addr+1)%NUM_PLANES] = {(row_addr[plane_addr][(ROW_BITS -1) : (PAGE_BITS)]+1),{PAGE_BITS{1'b0}}};
				    queued_plane_copy[(plane_addr+1)%NUM_PLANES] = 1'b1;
                                    active_plane = (plane_addr+1)%NUM_PLANES;
                                    cache_seqtl_cross_blk = 1'b1;
				end
                           end
                        end
                        if (cache_seqtl_cross_blk) begin
		      	   for (i=0;i<NUM_PLANES; i=i+1) begin
                              queued_plane[i] = queued_plane_copy[i];
                              if (queued_plane_copy[i]) begin
                                 row_addr[i] = row_addr_bk[i];
			      end
			   end
                           for (i = 0; i < NUM_PLANES; i = i +1) begin
                              	if (queued_plane_cache[i]) begin
	                         cache_rd_active_plane = i; // store this value so that the correct plane will output data during cache read
                        	end
                                queued_plane_cache[i] <= #tWB_delay queued_plane[i]; // delay this to allow the data_reg to be correctly copied to cache_reg during rd cache
                          end
...
*/
		...
		addr_cnt_en = 1; // enable counter b/c it was possibly disabled by previous 05h cmd during multi-lun sharing same p_ce config
            ...
            // Only 1 LUN returns data, to avoid collisions
            // ********************************************************
            else if ((Io [7 : 0] === 8'hEC) & (FEATURE_SET[CMD_ONFI] || FEATURE_SET2[CMD_JEDEC])) begin
	      if (id_cmd_lun) begin
	        die_select = 1'b1;
                cmnd_85h = 1'b0;
                saw_cmnd_00h = 1'b0;
		...
		read_out = 1'b0;
            ...
            // Only 1 LUN returns data, to avoid collisions
            // ********************************************************
            else if ((Io[7:0] === 8'hED) & FEATURE_SET[CMD_UNIQUE] & FEATURE_SET[CMD_ONFI]) begin
	      if (id_cmd_lun) begin
	        die_select = 1'b1;
                cmnd_85h = 1'b0;
                saw_cmnd_00h = 1'b0;
		...
		read_out = 1'b0;
            ...
            // Only 1 LUN returns data, to avoid collisions
            // ********************************************************
            else if ((Io [7 : 0] === 8'hEE) & FEATURE_SET[CMD_FEATURES] & die_select) begin
                cmnd_85h = 1'b0;
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
                saw_cmnd_60h = 1'b0;
    ...
    if (die_select && ~sync_mode && ~sync_enh_mode) begin : rd_die_select
        if (data_out_enable_async && (Rb_n_int === 1'b1)) begin : not_busy
	      ...
	      if (read_out) begin                
                ...
                $sformat(msg, "DATA NOT Transfered on Re_n,a CHANGE READ COLUMN (05h-E0h) or CHANGE READ COLUMN ENHANCED (06h-E0h) command shall be issued");
                WARN(msg);
	      end
...
always @ (posedge Re_n) begin
	...
	if (($realtime +tRHOH_min) < (tm_re_n_f + tREA_max))
	rd_out <= #(tm_re_n_f+tREA_max -$realtime + 0.5) 1'b0;
	else
...
begin
    ...
    while (delay > 0) begin
        ...
        last_we_n = We_n;
        if (Cle && We_n && ~Ale && Re_n && ~Ce_n && saw_edge_we_n) begin
	   if ((Io [7 : 0] !== 8'h78) && (Io [7 : 0] !== 8'h70)) begin  // tWB check should be ok if we have status command or status enhanced command in between
